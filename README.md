...
On macOS vimb is a native Objective-C + AppKit + WKWebView browser. The build
selects this backend automatically:

    make             # build vimb.app
    make runsandbox  # build and open vimb.app

The default opened page is the selected search engine's main page (see
...
(Safari-style) and the command line is a flat input docked at the bottom
edge, above a status line showing the current URL. Completions appear in a
dropdown floating above the command line, driven with `Tab`/`S-Tab` or the
arrow keys.
...
and a keymapping engine). Settings and history are stored under
`~/Library/Application Support/vimb` and the rc config file is sourced from
there on startup. It comes with a retro, dithered app icon (regenerated by
`make` from `macos/make_icon.swift`).

### Tests

`make test` runs the platform-appropriate suite and reports line coverage via
`llvm-cov` (on macOS this delegates to `make -C macos test`):

    make -C macos test

The tests use a lightweight custom harness (`macos/tests/`) with no external
dependencies (no Xcode/XCTest required). They cover the settings registry,
storage, mapping/shortcut engines, ex-commands, autocmds, and vim normal-mode
key parsing, including behavior cross-referenced against the original vimb
C tests (`tests/`). Coverage of the testable Foundation classes is ~96%.
